  ...
  function explodeStatement(node) {
    switch(node.type) {
      case "ExpressionStatement":
        //Special case: for simple assignments just flatten expression
        if(node.expression.type === "AssignmentExpression" &&
           node.expression.operator === "=") {
          if(node.expression.left.needsExplode) {
            if(node.expression.right.needsExplode) {
              var result = ["{"]
              result.push(explodeExpression(node.expression.right, explodeLValue(node.expression.left, result)))
              result.push("}\n")
              node.update(result.join("\n"))
            } else {
              var result = ["{"]
              result.push(explodeLValue(node.expression.left, result) + "=" + node.expression.right.source())
              result.push("}\n")
              node.update(result.join("\n"))
            }
          } else {
          ...
          //Second special case: inline function calls
          nvariables.push(block.ret)
          node.update([
            "{",
              inlineClosure(node.expression.arguments, block.ret),
            "}\n"
          ].join("\n"))
        } else {
